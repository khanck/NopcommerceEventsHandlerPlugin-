...
{
    public class OnOrderCancelledEvent : IConsumer<OrderCancelledEvent>
    {
        ...
        private async Task ProcessCustomFunctionAsync(Order order)
        {
            try
            {
                ...
                foreach (var orderItem in itemList)
                {
                    ...
                    if ((product.Sku != null)) 
                    {
                        // call any function you need to excecute the action after OrderCancelled
